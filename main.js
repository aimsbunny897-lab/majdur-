...
const CONFIG = {
  ...
  payment: {
    recipient: process.env.TREEBOT_PAY_RECIPIENT || 'ShadowX_Prime',
    primaryAmount: '150k',
    fallbackAmount: '80k',
    minDelayMs: 60 * 60 * 1000,
    maxDelayMs: 70 * 60 * 1000,
    responseTimeoutMs: 15000,
    commandCooldownMs: 2500,
    insufficientPatterns: [
      /not\s+enough\s+(?:money|funds|balance)/i,
      /insufficient\s+(?:money|funds|balance)/i,
      /you\s+(?:do\s+)?not\s+have\s+enough/i,
      /cannot\s+afford/i,
      /need\s+more\s+(?:money|funds)/i
    ],
    successPatterns: [
      /(?:payment|pay|transfer|transaction|money).*(?:successful|success|sent|received|completed)/i,
      /successfully\s+(?:paid|sent|transferred)/i,
      /(?:paid|sent|transferred)\s+.*(?:to|player)/i
    ]
  }
};
...
let paymentResponseTimer = null;
let paymentBusy = false;
let paymentAttemptId = 0;
let paymentGeneration = 0;
let paymentLastCommandAt = 0;
let paymentWaitingFor = null; // { amount, phase, generation, attemptId }
...
function paymentDelayMs() {
  return randomInt(CONFIG.payment.minDelayMs, CONFIG.payment.maxDelayMs);
}
...
function clearPaymentTimers() {
  ...
  if (paymentResponseTimer) {
    clearTimeout(paymentResponseTimer);
    paymentResponseTimer = null;
  }
}

function resetPaymentStateForConnection() {
  paymentGeneration++;
  paymentBusy = false;
  paymentWaitingFor = null;
  paymentLastCommandAt = 0;
  clearPaymentTimers();
}

function scheduleNextPayment(reason = 'cycle complete') {
  if (stopping || !authComplete || !isReady()) return;
  if (paymentTimer) clearTimeout(paymentTimer);

  const delay = paymentDelayMs();
  const generation = paymentGeneration;
  log(`[PAY] Next payment cycle in ${formatDuration(delay)} (${reason}).`);

  paymentTimer = setTimeout(() => {
    paymentTimer = null;
    if (stopping || generation !== paymentGeneration || !authComplete || !isReady() || paymentBusy) return;

    runPaymentCycle().catch(err => {
      state.stats.errors++;
      error('[PAY] Cycle error:', err.stack || err.message);
      paymentBusy = false;
      paymentWaitingFor = null;
      scheduleNextPayment('cycle error');
    });
  }, delay);
}

function paymentText(value) {
  return textOf(value).replace(/\u0000/g, ' ').replace(/\s+/g, ' ').trim();
}

function isPaymentInsufficient(text) {
  return CONFIG.payment.insufficientPatterns.some(re => re.test(text));
}

function isPaymentSuccess(text) {
  if (CONFIG.payment.successPatterns.some(re => re.test(text))) return true;

  // Many economy plugins use concise messages such as:
  // "You paid ShadowX_Prime 150k" or "Sent 150k to ShadowX_Prime".
  // Accept those only when the configured recipient is present and the message
  // contains an explicit payment verb; reject failure/error wording first.
  const recipient = String(CONFIG.payment.recipient || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  if (!recipient || !new RegExp(`\\b${recipient}\\b`, 'i').test(text)) return false;
  if (/(?:failed|failure|error|denied|rejected|cancelled|canceled|not enough|insufficient|cannot|can't|unable)/i.test(text)) {
    return false;
  }
  return /\\b(?:paid|pay|sent|transferred|received)\\b/i.test(text);
}

function sendPaymentCommand(amount) {
  if (stopping || !bot || !isReady()) throw new Error('Bot is not connected.');

  const nowMs = now();
  if (nowMs - paymentLastCommandAt < CONFIG.payment.commandCooldownMs) {
    throw new Error('Payment command cooldown is active.');
  }

  const recipient = String(CONFIG.payment.recipient || '').trim();
  const cleanAmount = String(amount || '').trim();

  if (!/^[A-Za-z0-9_]{1,16}$/.test(recipient)) throw new Error('Invalid payment recipient.');
  if (!/^\d+(?:\.\d+)?[kKmMbB]?$/.test(cleanAmount)) throw new Error('Invalid payment amount.');

  bot.chat(`/pay ${recipient} ${cleanAmount}`);
  paymentLastCommandAt = nowMs;
  action(`[PAY] Sent /pay ${recipient} ${cleanAmount}`);
}

function armPaymentResponseTimeout(waiting) {
  if (paymentResponseTimer) clearTimeout(paymentResponseTimer);

  paymentResponseTimer = setTimeout(() => {
    paymentResponseTimer = null;
    if (
      stopping ||
      !paymentBusy ||
      !paymentWaitingFor ||
      paymentWaitingFor.attemptId !== waiting.attemptId ||
      paymentWaitingFor.generation !== paymentGeneration
    ) return;

    // A timeout is deliberately NOT treated as insufficient funds.
    log(`[PAY] No definitive server response for ${waiting.amount} within ${CONFIG.payment.responseTimeoutMs / 1000}s; no fallback sent.`);
    paymentBusy = false;
    paymentWaitingFor = null;
    scheduleNextPayment('no definitive response');
  }, CONFIG.payment.responseTimeoutMs);
}

async function handlePaymentServerText(value, source = 'server') {
  const text = paymentText(value);
  if (!text || stopping || !paymentBusy || !paymentWaitingFor) return false;

  const waiting = paymentWaitingFor;
  if (waiting.generation !== paymentGeneration || waiting.attemptId !== paymentAttemptId) return false;

  if (isPaymentInsufficient(text)) {
    if (paymentResponseTimer) {
      clearTimeout(paymentResponseTimer);
      paymentResponseTimer = null;
    }

    if (waiting.phase === 'primary') {
      log(`[PAY] Server reported insufficient funds after 150k (${source}). Sending fallback 80k.`);
      paymentWaitingFor = {
        amount: CONFIG.payment.fallbackAmount,
        phase: 'fallback',
      ...
      try {
        ...
        if (
          stopping ||
          !paymentBusy ||
          paymentWaitingFor.attemptId !== paymentAttemptId ||
          paymentWaitingFor.generation !== paymentGeneration ||
          !isReady()
        ) return true;

        sendPaymentCommand(CONFIG.payment.fallbackAmount);
        armPaymentResponseTimeout(paymentWaitingFor);
      } catch (err) {
        state.stats.errors++;
        error('[PAY] Fallback 80k failed:', err.message);
        paymentBusy = false;
        paymentWaitingFor = null;
        scheduleNextPayment('fallback send failed');
      }
      return true;
    }

    log('[PAY] Server still reports insufficient funds after 80k; payment cycle ended.');
    paymentBusy = false;
    paymentWaitingFor = null;
    scheduleNextPayment('80k insufficient');
    return true;
  }

  if (isPaymentSuccess(text)) {
    if (paymentResponseTimer) {
      clearTimeout(paymentResponseTimer);
      paymentResponseTimer = null;
    }

    log(`[PAY] Server confirmed ${waiting.amount} payment successfully (${source}).`);
    paymentBusy = false;
    paymentWaitingFor = null;
    scheduleNextPayment('payment confirmed');
    return true;
  }
...
async function runPaymentCycle() {
  if (stopping || paymentBusy || !state.enabled || !authComplete || !isReady()) return;
  ...
  paymentWaitingFor = {
    amount: CONFIG.payment.primaryAmount,
    phase: 'primary',
    generation: paymentGeneration,
    attemptId
  };
  ...
  try {
    sendPaymentCommand(CONFIG.payment.primaryAmount);
    armPaymentResponseTimeout(paymentWaitingFor);
  } catch (err) {
    state.stats.errors++;
    error('[PAY] 150k payment failed to send:', err.message);
    paymentBusy = false;
    paymentWaitingFor = null;
    scheduleNextPayment('150k send failed');
  }
}
...
function startPaymentScheduler() {
  ...
  paymentBusy = false;
  paymentWaitingFor = null;
  scheduleNextPayment('authenticated');
}
...
function connect() {
  ...
  captchaDialog = null;
  queuedAuthPackets = [];
  resetPaymentStateForConnection();
  clearAuthTimers();
  if (authGateTimer) {
  ...
  try {
    ...
    bot.on('message', msg => {
      ...
      if (!text) return;
      handlePaymentServerText(text, 'message').catch(err => {
        state.stats.errors++;
        error('[PAY] Message handler:', err.message);
      });
    ...
    bot.on('messagestr', (text) => {
      handlePaymentServerText(text, 'messagestr').catch(err => {
        state.stats.errors++;
        error('[PAY] Messagestr handler:', err.message);
      });
      if (textLooksLikeAuthSuccess(text)) {
    ...
    bot.on('actionBar', (jsonMsg) => {
      ...
      handlePaymentServerText(text, 'action bar').catch(err => {
        state.stats.errors++;
        error('[PAY] Action-bar handler:', err.message);
      });
    ...
    bot.on('title', (title) => {
      ...
      handlePaymentServerText(text, 'title').catch(err => {
        state.stats.errors++;
        error('[PAY] Title handler:', err.message);
      });
    ...
    bot.on('systemChat', packet => {
      ...
      handlePaymentServerText(text, 'system chat').catch(err => {
        state.stats.errors++;
        error('[PAY] System-chat handler:', err.message);
      });
      if (textLooksLikeAuthSuccess(text)) {
...
function selfTest() {
  ...
  assert(CONFIG.payment.recipient === (process.env.TREEBOT_PAY_RECIPIENT || 'ShadowX_Prime'), 'payment recipient');
  assert(CONFIG.payment.primaryAmount === '150k', 'primary payment amount');
  assert(CONFIG.payment.fallbackAmount === '80k', 'fallback payment amount');
  assert(CONFIG.payment.minDelayMs === 60 * 60 * 1000, 'payment minimum delay is 60 minutes');
  assert(CONFIG.payment.maxDelayMs === 70 * 60 * 1000, 'payment maximum delay is 70 minutes');
  const delayCheck = paymentDelayMs();
  assert(delayCheck >= CONFIG.payment.minDelayMs && delayCheck <= CONFIG.payment.maxDelayMs, 'payment delay range');
  assert(isPaymentInsufficient('Not enough money'), 'insufficient-money detection');
  assert(isPaymentInsufficient('Insufficient funds'), 'insufficient-funds detection');
  assert(!isPaymentInsufficient('Payment successful'), 'success is not insufficient');
  assert(isPaymentSuccess('Payment successful'), 'payment success detection');
  assert(isPaymentSuccess('Successfully paid ShadowX_Prime'), 'paid success detection');
  assert(!isPaymentSuccess('Not enough money'), 'insufficient is not success');
  assert(/^ShadowX_Prime$/.test(CONFIG.payment.recipient), 'safe payment recipient');
  assert(/^150k$/.test(CONFIG.payment.primaryAmount), 'safe primary amount');
  assert(/^80k$/.test(CONFIG.payment.fallbackAmount), 'safe fallback amount');
  ...
  console.log('[SELFTEST] 60–70 minute payment scheduler = PASS');
  console.log('[SELFTEST] 150k -> insufficient only -> 80k fallback = PASS');
  console.log('[SELFTEST] Payment response timeout never assumes insufficient = PASS');
